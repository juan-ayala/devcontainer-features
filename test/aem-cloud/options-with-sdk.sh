...
check "sdk version default" \
    [ "${AEM_CLOUD_VERSION}" = "mock-2024.02.01" ]
check "sdks directory default" \
    [ "${AEM_CLOUD_SDKS_DIRECTORY}" = ".devcontainer/sdksfolder" ]
check "author port default" \
    [ "${AEM_CLOUD_AUTHOR_PORT}" = "3001" ]
check "publish port default" \
    [ "${AEM_CLOUD_PUBLISH_PORT}" = "3002" ]
check "dispatcher port default" \
    [ "${AEM_CLOUD_DISPATCHER_PORT}" = "8090" ]
